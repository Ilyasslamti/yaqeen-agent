...
# ==========================================
SYSTEM_VERSION = "V19.1_INVESTIGATIVE_FIXED"
ACCESS_PASSWORD = "Manager_Tech_2026"
DB_FILE = "news_db_v19.json"

st.set_page_config(page_title="وكيل يقين | المحرر الاستقصائي", page_icon="🕵️‍♂️", layout="wide")
socket.setdefaulttimeout(40)

# ==========================================
# 1. محرك الهندسة الصحفية الاستقصائية (The Beast Engine)
# ==========================================
def run_investigative_writer(text, tone, keyword):
    try:
        client = Groq(api_key=st.secrets["GROQ_API_KEY"])
        # برومبت يركز على العمق، الترابط، واللغة الراقية بعيداً عن التهريج
        prompt = f"""
        ...
        بروتوكول الماندجر إلياس:
        1. لغة النخبة: ابْنِ فقرات مترابطة طويلة النفس، واستخدم الفواصل (،) لخلق تدفق فكري مستمر بعيداً عن الجمل المقطوعة.
        2. التحليل لا السرد: اربط الظواهر ببعضها وقدم تحليلاً للأسباب والنتائج.
        3. هيكلة SEO: العنوان يجب أن يكون قوياً ويبدأ بـ ({keyword})، مع عناوين فرعية (H2) تحليلية بدون رموز.
        4. الروابط اللغوية: استخدم روابط فخمة (وتأسيساً على ما تقدم، وفي مقلب آخر من المشهد، بيد أن التدقيق يكشف..).
        5. منع التكرار: ادمج المعلومات المتشابهة وقدم نصاً دَسِماً يليق بمجموعة الماندجر.
        
        الأسلوب: {tone}. الكلمة المفتاحية: {keyword}.
        النص الأصلي: {text[:4000]}
        """
...
# ==========================================
# 3. القائمة الكاملة (53 مصدراً) - حذارِ من الحذف
# ==========================================
RSS_SOURCES = {
    "الصحافة الوطنية 🇲🇦": {
...
st.title("🕵️‍♂️ وكيل يقين | المحرر الاستقصائي")
            ...
            if st.button("🚀 صياغة المقال الاستراتيجي", key=f"run_{i}"):
                with st.spinner("جاري الهندسة الصحفية..."):
                    ...
                    if txt:
                        ...
                        st.markdown("### ✅ المقال الاستقصائي الجاهز")
...
st.markdown("---")
st.caption("وكيل يقين V19.1 - إدارة الماندجر 2026 - الإصدار الاستقصائي النخبوي")
