...
def rewrite_seo_architect(text, tone, keyword):
    if not client: return "خطأ في الاتصال بالذكاء الاصطناعي (تأكد من مفتاح API)"
    ...
    1. العنوان الرئيسي: صغ عنواناً نصياً (بدون رموز) "مغناطيسياً" يبدأ بالكلمة المفتاحية.
    2. المقدمة: فقرة افتتاحية (30 كلمة) تحتوي الكلمة المفتاحية وتلخص الحدث.
    3. العناوين الفرعية: قسّم المقال بعناوين نصية واضحة (بدون رموز).
    4. معايير Yoast SEO: 
       - استخدم كلمات انتقال بكثافة (علاوة على ذلك، في المقابل، ومن جهة أخرى).
       - الجمل قصيرة جداً (أقل من 18 كلمة).
       - المبني للمعلوم حصراً.
       - الفقرات قصيرة (3 أسطر بحد أقصى).
    
    الأسلوب: {tone}. النص الأصلي: {text[:3800]}
    """
    try:
    ...
    for i, cat in enumerate(list(RSS_SOURCES.keys())):
        with tabs[i]:
            ...
            if cat in db["data"] and db["data"][cat]:
                ...
                with c2: keyword = st.text_input("الكلمة المفتاحية (SEO):", key=f"k_{i}")

                if st.button("🚀 توليد مقال احترافي متصدر", key=f"r_{i}"):
                    with st.status("🏗️ جاري بناء المقال...", expanded=True):
                        ...
                        if txt:
                            ...
                            st.text_area("نسخة النشر:", final_content, height=450)
                        else: st.error("المصدر يمنع السحب")
            else: st.info("اضغط تحديث لجلب البيانات.")

    st.markdown("---")
    st.markdown("<p style='text-align:center; color:#666;'>وكيل يقين الصحفي V16.0 - تطوير وحلول الماندجر</p>", unsafe_allow_html=True)                else: st.error("❌ المفتاح غير صحيح")
        return False
    return True
