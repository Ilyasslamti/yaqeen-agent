...
def rewrite(text, tone, instr):
    # برومبت السيو المتقدم (SEO Prompt)
    prompt = f"""
    ...
    - اكتب وصف ميتا (Meta Description) دقيق (150 حرف).
...
with st.sidebar:
    ...
    # === القسم الجديد: عرض الجرائد ===
    # هذا هو القسم "الإضافي في الشمال" الذي طلبته
    current_sources = list(RSS_SOURCES[cat].keys())
    with st.expander(f"👁️ عرض الجرائد ({len(current_sources)})", expanded=True):
        # عرض الجرائد كـ "Tags" أنيقة
    ...
    # 2. التحكم في البحث
    st.markdown("### 📊 نطاق التغطية")
    limit = st.slider("عدد الأخبار/جريدة:", 5, 30, 15, help="زيادة العدد تعني الرجوع أكثر في الوقت")
    
    # 3. إعدادات المحرر
    st.markdown("### ✍️ المحرر الذكي")
    tone = st.select_slider("النبرة:", ["رسمي", "تحليلي", "تفاعلي"])
    ins = st.text_input("توجيهات خاصة:")
...
if news:
    count = len(news)
    st.success(f"تم رصد **{count}** مقالاً من {len(current_sources)} مصدراً في قسم {cat}.")
    ...
    st.info("اضغط 'تحديث المصادر' للبدء...")        "كاب 24": "https://cap24.tv/feed",
        "طنجة نيوز": "https://tanjanews.com/feed",
        "صدى تطوان": "https://sadatetouan.com/feed",
    },
    "📰 صحف وطنية": {
