...
html, body, .stApp { font-family:'Tajawal', sans-serif; background: var(--bg); color: var(--text); }
body { direction: rtl; }

/* اترك هيدر Streamlit ظاهرًا كي يبقى زر إظهار/إخفاء القائمة الجانبية شغال */
...
/* اجعل زر إظهار القائمة الجانبية واضحًا دائماً */
...
.badge{display:inline-flex;align-items:center;gap:6px;padding:6px 10px;border-radius:999px;border:1px solid var(--border);background: rgba(29,78,216,.06);color: var(--brand2);font-size:12px;font-weight:700;}
.news-card{background:var(--card);border:1px solid var(--border);border-radius:18px;box-shadow:var(--shadow);margin-bottom:10px;}
.news-title{font-weight:800;color:var(--text);}
.news-meta{display:flex;gap:14px;flex-wrap:wrap;color:var(--muted);font-size:12px;margin-top:6px;}
...
.brand-title{font-weight:900;font-size:1.15rem;color:var(--text);}
...
.live-dot{width:8px;height:8px;border-radius:99px;background:#16a34a;box-shadow:0 0 0 4px rgba(34,197,94,.15);}
.badge-dot{width:6px;height:6px;border-radius:99px;background:var(--brand);display:inline-block;margin-inline:6px;}
...
if st.session_state.page == 'public':
    ...
    with st.sidebar:
        ...
        st.divider()
        selected_cat = st.radio("الأقسام", list(RSS_DATABASE.keys()), label_visibility="collapsed", key="selected_cat")
    ...
    # شريط تحكم ظاهر حتى لو كانت القائمة الجانبية مخفية
    cols_top = st.columns([2, 3, 2])
    with cols_top[0]:
        st.markdown("### 🗞️ العناوين")
    with cols_top[1]:
        current = st.session_state.get("selected_cat", list(RSS_DATABASE.keys())[0])
        selected_cat_main = st.selectbox("القسم", list(RSS_DATABASE.keys()), index=list(RSS_DATABASE.keys()).index(current), label_visibility="collapsed", key="selected_cat_select_public")
        if selected_cat_main != current:
            st.session_state["selected_cat"] = selected_cat_main
            selected_cat = selected_cat_main
    with cols_top[2]:
        st.write("")
    st.markdown(f"<div class='yaq-muted' style='margin-top:-6px'>قسم: <b>{selected_cat}</b> — عرض العناوين فقط</div>", unsafe_allow_html=True)
    ...
    if news_list:
        ...
        page_size = st.selectbox("عدد العناوين في الصفحة", [20, 50, 100, 200], index=0, key="public_page_size")
        total_pages = max(1, math.ceil(len(news_list) / page_size))
        page = st.number_input("الصفحة", min_value=1, max_value=total_pages, value=1, step=1, key="public_page")
