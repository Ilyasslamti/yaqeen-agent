...
/* Typography & contrast (critical for readability) */
html, body, .stApp{color: var(--text) !important;}
.stMarkdown, .stCaption, .stText, .stAlert, label, p, span, li{color: var(--text);}
.stMarkdown a{color: var(--brand) !important; text-decoration: none;}
.stMarkdown a:hover{text-decoration: underline;}

/* Form controls: keep text/placeholder visible */
input, textarea{color: var(--text) !important;}
input::placeholder, textarea::placeholder{color: rgba(11,18,32,.45) !important;}
div[data-baseweb="input"] input{background: rgba(255,255,255,.92) !important;}
div[data-baseweb="select"] > div{background: rgba(255,255,255,.92) !important; color: var(--text) !important; border-color: var(--border) !important;}
div[data-baseweb="select"] svg{fill: var(--muted) !important;}

/* Hero: force readable title */
.yq-hero{color: var(--text) !important;}
.yq-hero h1{color: var(--text) !important; text-shadow: 0 1px 0 rgba(255,255,255,.75);}
.yq-hero p{color: var(--muted) !important;}
...
/* Landing upgrades */
.yq-chip{display:inline-flex; align-items:center; gap:.45rem; font-weight:800; color:var(--text);
  background: rgba(255,255,255,.78); border:1px solid var(--border); border-radius: 999px;
  padding: .42rem .75rem; box-shadow: 0 6px 18px rgba(2,6,23,.06);}
.yq-ic{width:18px; height:18px; fill: currentColor; opacity:.95}
.yq-hero h1{background: linear-gradient(90deg, var(--brand), var(--brand2), var(--brand3));
  -webkit-background-clip:text; background-clip:text; color: transparent !important; text-shadow:none !important;}
.yq-hero p b{color:var(--text);}
.yq-trust{display:flex; flex-wrap:wrap; gap:.55rem; margin-top: 14px;}
.yq-badge{display:inline-flex; align-items:center; gap:.45rem;
  border:1px solid var(--border); background: rgba(255,255,255,.72);
  padding:.45rem .65rem; border-radius: 999px; color: var(--text); font-weight:800; font-size:.95rem;}
.yq-badge small{color:var(--muted); font-weight:700;}
.yq-features{display:grid; grid-template-columns: repeat(3, minmax(0,1fr)); gap: 14px; margin-top: 16px;}
.yq-feature{background: rgba(255,255,255,.86); border:1px solid var(--border); border-radius: 18px;
  padding: 14px 14px; box-shadow: 0 10px 26px rgba(2,6,23,.06);}
.yq-feature h3{margin:.2rem 0 .2rem 0; font-size: 1.05rem; color: var(--text);}
.yq-feature p{margin:0; color: var(--muted); line-height:1.75; font-size:.98rem;}
.yq-feature .yq-ic{width:20px; height:20px;}
.yq-steps{display:grid; grid-template-columns: repeat(3, minmax(0,1fr)); gap: 12px; margin-top: 14px;}
.yq-step{border:1px dashed rgba(15,23,42,.18); background: rgba(255,255,255,.70); border-radius: 18px; padding: 12px 12px;}
.yq-step b{display:block; color: var(--text); margin-bottom: 4px;}
.yq-step span{color: var(--muted); font-weight:700;}
@media (max-width: 900px){
  .yq-features,.yq-steps{grid-template-columns: 1fr;}
  .yq-kpi{min-width: 100%;}
}
...
if st.session_state.page == 'public':
    ...
    with nav_l:
        st.button("🗞️ العناوين", use_container_width=True, disabled=True, key="nav_public_buttons")
    with nav_r:
        if st.button("⚡ دخول غرفة التحرير", use_container_width=True, type="primary", key="nav_to_newsroom"):
            goto_page("newsroom")
    ...
    st.markdown(
        """
        <div class="yq-hero">
          <div class="yq-chip"><svg class="yq-ic" viewBox="0 0 24 24" aria-hidden="true"><path d="M12 2a10 10 0 100 20 10 10 0 000-20zm7.9 9h-3.1a15.5 15.5 0 00-1.2-5 8.1 8.1 0 014.3 5zM12 4c.9 1.2 1.7 3 2.2 7H9.8c.5-4 1.3-5.8 2.2-7zm-6 7H3.1a8.1 8.1 0 014.3-5c-.6 1.5-1 3.2-1.4 5zm0 2c.3 1.9.8 3.7 1.4 5a8.1 8.1 0 01-4.3-5H6zm3.8 0h4.4c-.5 4-1.3 5.8-2.2 7-.9-1.2-1.7-3-2.2-7zm6 0h3.1a8.1 8.1 0 01-4.3 5c.6-1.5 1-3.2 1.2-5z"/></svg><small>Yaqeen Press</small><span style="opacity:.6">|</span><small>Newsroom Console</small></div>
          <h1>السيادة على الخبر… بسرعة غرفة أخبار عالمية</h1>
          <p>
            منصة تحرير تجمع العناوين من مصادر متعددة، وتمنحك <b>استخلاصًا</b> و<b>إعادة صياغة</b> و<b>تصديرًا</b> عند تسجيل الدخول.
            <br/>كزائر: <b>عناوين فقط</b>. كعضو: <b>تحرير احترافي</b> + <b>حفظ وتصدير</b>.
          </p>

          <div class="yq-trust">
            <div class="yq-badge"><svg class="yq-ic" viewBox="0 0 24 24" aria-hidden="true"><path d="M13 2L3 14h7l-1 8 12-14h-7z"/></svg><span>سريع</span><small>تحديث ذكي</small></div>
            <div class="yq-badge"><svg class="yq-ic" viewBox="0 0 24 24" aria-hidden="true"><path d="M12 2l8 4v6c0 5-3.4 9.7-8 10-4.6-.3-8-5-8-10V6z"/></svg><span>آمن</span><small>عضويات</small></div>
            <div class="yq-badge"><svg class="yq-ic" viewBox="0 0 24 24" aria-hidden="true"><path d="M7 2h10c1.1 0 2 .9 2 2v16c0 1.1-.9 2-2 2H7c-1.1 0-2-.9-2-2V4c0-1.1.9-2 2-2zm0 4v12h10V6z"/></svg><span>متوافق جوال</span><small>Responsive</small></div>
            <div class="yq-badge"><svg class="yq-ic" viewBox="0 0 24 24" aria-hidden="true"><path d="M3 17.2V21h3.8L19.8 8l-3.8-3.8zM21 6.8c.4-.4.4-1 0-1.4L18.6 3c-.4-.4-1-.4-1.4 0l-1.8 1.8L19.2 8.6z"/></svg><span>تحرير</span><small>SEO-friendly</small></div>
          </div>
          ...
          <div class="yq-features">
            <div class="yq-feature"><svg class="yq-ic" viewBox="0 0 24 24" aria-hidden="true"><path d="M12 2a10 10 0 100 20 10 10 0 000-20zm7.9 9h-3.1a15.5 15.5 0 00-1.2-5 8.1 8.1 0 014.3 5zM12 4c.9 1.2 1.7 3 2.2 7H9.8c.5-4 1.3-5.8 2.2-7zm-6 7H3.1a8.1 8.1 0 014.3-5c-.6 1.5-1 3.2-1.4 5zm0 2c.3 1.9.8 3.7 1.4 5a8.1 8.1 0 01-4.3-5H6zm3.8 0h4.4c-.5 4-1.3 5.8-2.2 7-.9-1.2-1.7-3-2.2-7zm6 0h3.1a8.1 8.1 0 01-4.3 5c.6-1.5 1-3.2 1.2-5z"/></svg><h3>تجميع مصادر متعدد</h3><p>نستخرج العناوين من عشرات المصادر ونرتبها بسرعة داخل لوحة واحدة.</p></div>
            <div class="yq-feature"><svg class="yq-ic" viewBox="0 0 24 24" aria-hidden="true"><path d="M3 17.2V21h3.8L19.8 8l-3.8-3.8zM21 6.8c.4-.4.4-1 0-1.4L18.6 3c-.4-.4-1-.4-1.4 0l-1.8 1.8L19.2 8.6z"/></svg><h3>محرر غرفة أخبار</h3><p>استخلاص + إعادة صياغة باحتراف، مع معاينة وتصدير جاهز للنشر.</p></div>
            <div class="yq-feature"><svg class="yq-ic" viewBox="0 0 24 24" aria-hidden="true"><path d="M12 2l8 4v6c0 5-3.4 9.7-8 10-4.6-.3-8-5-8-10V6z"/></svg><h3>نظام عضويات وباقات</h3><p>Free يومي محدود، وPro مفتوح — مع تتبع الاستعمال وحماية مفاتيحك.</p></div>
          </div>

          <div class="yq-steps">
            <div class="yq-step"><b>1) اختر القسم</b><span>ثم اعرض العناوين فقط.</span></div>
            <div class="yq-step"><b>2) سجّل الدخول</b><span>لفتح الاستخلاص والتحرير.</span></div>
            <div class="yq-step"><b>3) صدّر للنشر</b><span>TXT/Markdown جاهزين.</span></div>
          </div>
        </div>
        """,
    ...
    # =============================
    # شريط التحكم (Public) — ظاهر دائماً + متوافق مع الجوال
    # =============================
    if "selected_cat" not in st.session_state:
        st.session_state["selected_cat"] = (list(RSS_DATABASE.keys())[0] if RSS_DATABASE else "")
    if "entries_per_source" not in st.session_state:
        st.session_state["entries_per_source"] = 10
    if "headlines_per_page" not in st.session_state:
        st.session_state["headlines_per_page"] = 50
    if "search_query" not in st.session_state:
        st.session_state["search_query"] = ""

    c1, c2, c3, c4 = st.columns([2.4, 1.2, 1.2, 1.4])
    with c1:
    ...
    with c3:
        st.number_input("عدد العناوين في الصفحة", min_value=10, max_value=200, value=int(st.session_state.get("headlines_per_page", 50)), step=10, key="headlines_per_page")
    with c4:
        st.text_input("بحث", key="search_query", placeholder="وزارة، ريال، لقاح…")

    r1, _ = st.columns([1.1, 3])
    with r1:
    ...
    # --- رجوع سريع للعناوين ---
    if st.button("← رجوع للعناوين", use_container_width=False, key="nav_back_public"):
        goto_page("public")
