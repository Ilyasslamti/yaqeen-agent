...
import socket
import requests
import manadger_lib  # <--- هنا يتم استيراد الملف السابق

# ==========================================
# 0. إعدادات النظام
# ==========================================
SYSTEM_VERSION = "V33.3_FIXED_IMPORT"
...
# ==========================================
# نسحب قاعدة البيانات من الملف الآخر
RSS_SOURCES = manadger_lib.RSS_DATABASE

# ==========================================
# 3. CSS (تصميم الماندجر)
# ==========================================
st.markdown("""
<style>
    ...
    .comparison-box {
        height: 600px; overflow-y: auto; padding: 20px; border-radius: 8px;
...
def get_groq_client():
    # استخدام نظام تدوير المفاتيح من المكتبة
...
def smart_rewrite(text, keyword):
    ...
    # دمج الكلمة المفتاحية مع البرومبت القسري من المكتبة
    try:
        # هنا نستدعي البرومبت العظيم ونحقن فيه الكلمة المفتاحية
        final_prompt = manadger_lib.ELITE_PROMPT.format(keyword=keyword)
        # نضيف النص في النهاية
        final_prompt += f"\n{text[:4000]}"
        
        res = client.chat.completions.create(
            messages=[{"role": "user", "content": final_prompt}],
            model="llama-3.3-70b-versatile", 
            temperature=0.5 # حرارة منخفضة للالتزام الصارم بالتعليمات
        )
...
<div class='brand-header'>
    ...
    <p style='color:#6c757d; margin-top:5px'>نظام التحرير الصارم V33.1</p>
</div>
""", unsafe_allow_html=True)
...
for i, cat_name in enumerate(cats):
    with tabs[i]:
        if "data" in db and cat_name in db["data"] and len(db["data"][cat_name]) > 0:
            ...
            # هنا التغيير: نطلب الكلمة المفتاحية إجبارياً كما في البرومبت
