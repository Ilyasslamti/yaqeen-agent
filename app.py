...
# ==========================================
SYSTEM_VERSION = "V16.5_STABLE_FULL" 
...
def check_password():
    ...
    if not st.session_state["authenticated"]:
        st.markdown("<div style='text-align: center; background: #1e3a8a; color: white; padding: 2rem; border-radius: 15px;'><h1>🔐 وكيل يقين الصحفي</h1><p>نظام السحب والصياغة الاحترافي - إدارة الماندجر</p></div>", unsafe_allow_html=True)
...
# ==========================================
# 3. تشغيل النظام والقائمة الكاملة للجرائد
# ==========================================
if check_password():
    ...
    <style>
        ...
        .article-output {
            ...
            border-radius: 12px; border: 1px solid #cfd8dc; line-height: 2.1; font-size: 1.15rem;
        }
    ...
    for i, cat in enumerate(list(RSS_SOURCES.keys())):
        with tabs[i]:
            ...
            if cat in db["data"] and db["data"][cat]:
                ...
                c1, c2 = st.columns(2)
                with c1:
                    tone = st.selectbox("الأسلوب الصحفي:", ["تحقيق رصين بجمل قصيرة", "تقرير إخباري سريع", "تحليل تفاعلي SEO"], key=f"tn_{i}")
                with c2:
                    keyword = st.text_input("الكلمة المفتاحية (SEO):", key=f"kw_{i}")

                if st.button("🚀 هندسة وصياغة المقال", key=f"run_{i}"):
                    with st.status("🏗️ جاري المعالجة وتطبيق معايير Yoast SEO...", expanded=True):
    ...
    st.markdown("---")
    st.markdown("<p style='text-align:center; color:#666;'>وكيل يقين الصحفي V16.5 - إدارة الماندجر 2026</p>", unsafe_allow_html=True)                else: st.error("❌ المفتاح غير صحيح")
        return False
    return True
