...
# ==========================================
# 0. إعدادات الهوية والتحصين (الماندجر V25.5)
# ==========================================
SYSTEM_VERSION = "V25.5_DISCIPLINED_JOURNALISM"
ACCESS_PASSWORD = "Manager_Tech_2026"
DB_FILE = "news_db_v25.json"

st.set_page_config(page_title="يقين AI | الصياغة المنضبطة", page_icon="✒️", layout="wide")
socket.setdefaulttimeout(40)

# ==========================================
# 1. محرك الصياغة الصحفية النخبوية (البرومت الجديد)
# ==========================================
def run_samba_writer(text, tone, keyword):
    try:
        ...
        # دمج برومت الصياغة الصحفية النخبوية المنضبطة كما زوّدتني به حرفياً
        prompt = f"""
برومت الصياغة الصحفية النخبوية المنضبطة (نسخة مُصحَّحة تقنيًا)
المهمة:
...
يُمنع بدء أكثر من جملتين متتاليتين بنفس الكلمة أو التركيب. نوّع بدايات الجمل بين: (ظرف زمان، ظرف مكان، فعل مباشر، جملة وصفية، تركيب سياقي مثل: وفي هذا السياق، بالتوازي مع ذلك، من جهة أخرى…).

4. بنية الفقرة الصحفية:
كل فقرة يجب أن تتكون من فكرة واحدة مكتملة. تبدأ الفقرة بمدخل قوي واضح، وتُدعَّم بتفاصيل دقيقة دون حشو. يمنع الانتقال بين أفكار غير مترابطة داخل نفس الفقرة.

5. اللغة الصحفية الحديثة:
استخدم تعابير مهنية من قبيل: (وفي هذا السياق، فور وقوع الواقعة، حسب معطيات أولية، بالتوازي مع ذلك، وأسفر هذا الإجراء عن، ويعيد هذا المعطى إلى الواجهة).
...
يجب إدراج عناوين فرعية نصية فقط عند كل انتقال زاوية. الحد الأدنى: 3 عناوين فرعية إذا كان النص طويلاً.

8. نظافة النص:
يمنع منعًا باتًا استعمال: (النجوم، الهاشتاغات، الإيموجي، الأقواس البرمجية، أو أي رموز). النص يجب أن يكون صالحًا للطباعة في جريدة ورقية دون أي تعديل.

9. منع المقدمات الآلية:
لا تستخدم عبارات مثل: ("إليك المقال"، "فيما يلي"، "هذه الصياغة"). ابدأ مباشرة بالعنوان ثم المتن الصحفي.
...
"""
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": "أنت محرر صحفي نخبوّي تلتزم بقواعد الصياغة الصحفية الرصينة والمنضبطة."},
                {"role": "user", "content": prompt}
            ],
            temperature=0.4, # خفض الحرارة لزيادة الانضباط ومنع الاجتهاد
            top_p=0.9
        )
        return response.choices[0].message.content
    except Exception as e:
        return f"عذراً إلياس، حدث خطأ في محرك الصياغة: {str(e)}"
...
if not st.session_state["authenticated"]:
    st.markdown("<h2 style='text-align:center;'>🔐 دخول منصة يقين AI - إصدار الصياغة المنضبطة</h2>", unsafe_allow_html=True)
...
# ==========================================
# 3. الترسانة الإعلامية (65 مصدراً - خط أحمر لا يُلمس)
# ==========================================
RSS_SOURCES = {
    "الصحافة الوطنية 🇲🇦": {
...
# ==========================================
# 4. الواجهة والتنسيق (Premium Layout)
# ==========================================
st.markdown("""
<style>
...
st.title("🦅 يقين AI | الصياغة الصحفية النخبوية V25.5")
...
for i, cat in enumerate(list(RSS_SOURCES.keys())):
    with tabs[i]:
        if st.button(f"🔄 تحديث شامل لـ {cat}", key=f"up_{i}"):
            with st.spinner("جاري استحضار الترسانة الإعلامية الـ 65..."):
                all_news = []
                def fetch_t(n, u):
                    try:
        ...
        if cat in db["data"] and db["data"][cat]:
            ...
            choice = st.selectbox("اختر الخبر الخام للمقالة:", range(len(news_list)), format_func=lambda x: f"[{news_list[x]['source']}] {news_list[x]['title']}", key=f"sel_{i}")
            c1, c2 = st.columns(2)
            ...
            if st.button("🚀 صياغة المقال الاستراتيجي", key=f"run_{i}"):
                with st.spinner("جاري الكتابة بالبرومت المنضبط..."):
                    ...
                    if txt:
                        ...
                        st.text_area("نسخة النشر المباشر (خالية من الرموز):", final, height=500)
                    else: st.error("فشل في سحب محتوى الخبر.")
        else: st.info("اضغط تحديث لتفعيل المصادر الـ 65.")

st.markdown("---")
st.caption("يقين V25.5 - إدارة الماندجر إلياس - 65 مصدراً - برومت الصياغة المنضبطة 2026")
