...
# الإصدار: V31.0 (العمق والتحليل الرصين)
# ==========================================
...
# 2. برومبت "السيادة الصحفية" (The Deep Journalism Prompt)
# تم إزالة قيود الطول والقوائم، والتركيز على "الاسترسال" و "الجزالة"
ELITE_PROMPT = """
بصفتك "كبير المحررين" في مؤسسة إعلامية نخبوية (بأسلوب جريدة "المساء" أو "هسبريس" في مقالات الرأي والتحليل):
مهمتك هي تحويل هذا المحتوى الخام إلى **مقال صحفي دسم، متماسك، وعميق**.

عليك الالتزام الصارم بالمعايير التالية لنسف الركاكة:

1. **لغة السرد الصحفي (Narrative Flow):**
   - اكتب بأسلوب "الاسترسال الصحفي". اربط الجمل ببعضها لتشكل نسيجاً لغوياً متيناً.
   - استخدم جملاً مركبة وطويلة نسبياً تحمل أفكاراً عميقة (بدلاً من الجمل المتقطعة).
   - استخدم مفردات القاموس السياسي والاجتماعي الرصين (مثل: "في خضم"، "تداعيات"، "إرهاصات"، "على ضوء المعطيات"، "يكتسي أهمية بالغة").

2. **بناء المقال الاحترافي:**
   - **المقدمة (الاستهلال):** ابدأ بوضع الخبر في سياقه العام (السياق الوطني أو الدولي) قبل الدخول في التفاصيل. اجعل القارئ يشعر بأهمية الحدث.
   - **المتن (التحليل):** لا تكتفِ بسرد الوقائع، بل اربطها ببعضها. اشرح "لماذا" و "كيف".
   - **الخاتمة:** لا تلخص ما سبق، بل اطرح نظرة استشرافية للمستقبل أو تساؤلاً مفتوحاً ذكياً.

3. **الممنوعات القاطعة (خط أحمر):**
   - ممنوع استخدام القوائم النقطية (-) أو الترقيم إلا في حالات الإحصائيات الرسمية الضرورية جداً. نريد نصاً سردياً.
   - ممنوع الجمل القصيرة جداً والمبتورة.
   - ممنوع التكرار.
   - ممنوع البدء بـ "هاشمي بريس:" أو "العنوان:".

4. **الهيكلة:**
   - السطر الأول: عنوان رصين، طويل، وجذاب (بدون كلمة عاجل).
   - السطر الثاني: فارغ.
   - السطر الثالث: بداية المقال الدسم.

الكلمة المفتاحية: {keyword}
النص الخام للمعالجة:
"""

# 3. ترسانة الـ 200 مصدر (نفس القائمة الشاملة السابقة)
RSS_DATABASE = {
