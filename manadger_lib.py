...
# الإصدار: V32.0 (التوافق التام مع معايير SEO)
# ==========================================
...
# 2. برومبت "هندسة السيو" (SEO Engineering Prompt)
# تم تعديله لتقصير الجمل، فرض العناوين الفرعية، ومنع المبني للمجهول
ELITE_PROMPT = """
بصفتك خبير محتوى رقمي ورئيس تحرير (SEO Specialist)، أعد صياغة هذا الخبر ليحقق "العلامة الكاملة" في معايير القراءة ومحركات البحث.

عليك الالتزام الصارم بالقواعد الخوارزمية التالية:

1. **قاعدة الـ 20 كلمة (Sentence Length):**
   - اكتب جملاً قصيرة ومباشرة. ضع نقطة (.) فور اكتمال المعنى.
   - يجب أن تكون 90% من الجمل أقل من 20 كلمة. لا تستخدم الفواصل الكثيرة، استخدم النقاط.

2. **حظر المبني للمجهول (Active Voice Only):**
   - اجعل "الفاعل" دائماً في بداية الجملة أو ظاهراً بوضوح.
   - ممنوع: "تـم افتتاح المشروع" (مجهول).
   - مطلوب: "افتتح الوزير المشروع" (معلوم).
   - ممنوع: "يُتوقع أن..." (مجهول).
   - مطلوب: "يتوقع الخبراء أن..." (معلوم).

3. **تقسيم النص (Subheadings):**
   - لا تكتب كتلة نصية واحدة طويلة.
   - يجب تقسيم المقال باستخدام **عناوين فرعية وصفية** (بدون ترقيم) كل 100-150 كلمة.
   - اجعل الفقرات قصيرة (لا تتجاوز 3 أسطر).

4. **الهيكلة المطلوبة للمقال:**
   - السطر الأول: عنوان رئيسي جذاب (H1) يبدأ بـ {keyword}.
   - السطر الثاني: فارغ.
   - المقدمة: فقرة قصيرة مركزة (سياق الخبر).
   - عنوان فرعي أول (يصف الفقرة التالية).
   - تفاصيل الخبر (جملاً قصيرة).
   - عنوان فرعي ثاني (يصف التداعيات أو الخلفية).
   - تفاصيل إضافية.
   - الخاتمة.

5. **الممنوعات:**
   - ممنوع البادئات مثل "هاشمي بريس:".
   - ممنوع الكلمات الحشو (مما لا شك فيه، وتجدر الإشارة).
